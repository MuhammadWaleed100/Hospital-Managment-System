	...
	<?php
	 ...
	 ?>
		...
		<div class="statistics">
			...
			$year = date('Y');
				...
				while ($row = sqlsrv_fetch_array($result)) {
					$sql1 = sqlsrv_query($conn,"SELECT sum(test_price) AS bb FROM medication WHERE date='$day' AND month='$month' AND year='$year'",$params,$options);
					while ($row1 = sqlsrv_fetch_array($sql1)) {
						$sql2 =sqlsrv_query($conn,"SELECT sum(medical_price) AS cc FROM medication WHERE date='$day' AND month='$month' AND year='$year'",$params,$options);
			...
			$year = date('Y');
				...
				$result = sqlsrv_query($conn,$sql,$params,$options);
				while ($row = sqlsrv_fetch_array($result)) {
					$sql1 = sqlsrv_query($conn,"SELECT sum(test_price) AS bb FROM medication WHERE month='$month' AND year='$year'",$params,$options);
					while ($row1 = sqlsrv_fetch_array($sql1)) {
						$sql2 =sqlsrv_query($conn,"SELECT sum(medical_price) AS cc FROM medication WHERE month='$month' AND year='$year'",$params,$options);
						while ($row2 = sqlsrv_fetch_array($sql2)) {
							$all = $row['aa'] + $row1['bb'] + $row2['cc'];
							echo number_format($all);
						}
			...
			$year = date('Y');
				...
				$result = sqlsrv_query($conn,$sql,$params,$options);
				while ($row = sqlsrv_fetch_array($result)) {
					$sql1 = sqlsrv_query($conn,"SELECT sum(test_price) AS bb FROM medication WHERE year='$year'",$params,$options);
					while ($row1 = sqlsrv_fetch_array($sql1)) {
						$sql2 =sqlsrv_query($conn,"SELECT sum(medical_price) AS cc FROM medication WHERE year='$year'",$params,$options);
						while ($row2 = sqlsrv_fetch_array($sql2)) {
							$all = $row['aa'] + $row1['bb'] + $row2['cc'];
							echo number_format($all);
						}
		...
		if ($_SERVER['REQUEST_METHOD']=="POST") {
			...
			<tbody>
			<?php
				$sql = "SELECT * FROM medication WHERE date='$day' AND month='$month' AND year='$year' ORDER BY P_ID DESC";
				 	$result = sqlsrv_query($conn,$sql,$params,$options);
